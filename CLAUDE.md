...
- Modelo de datos (Prisma): settings, users (+ campos de promotor), refresh_tokens, password_recoveries, auth_challenges, devices, oauth_accounts, categories, events (+ `maxTransfers`), event_media, localities, seat_maps, seats, fee_schedules, orders (con campos FEL + feeScheduleVersion), order_items, **payment_gateways**, payments, webhook_events, ledger_accounts, ledger_transactions, ledger_entries, **tickets** (firma Ed25519 + totpSecret cifrado + media + transferCount), **ticket_custody_events** (hash-chain), **ticket_transfers**, **ticket_sync_entries**.
...
**Estado (jul 2026): Olas 0/1/1.5/2/3/3.5/4/5 COMPLETAS — 265 tests verdes (32 suites), 0 warnings de lint.** Ola 5 (transferencias + chain-of-custody + validación offline SafeTix + propagación de revocaciones) cerrada. Auditoría QA previa (tras Ola 4) dejó la base en 247 tests cerrando huecos (media/seat-maps sin tests, gestión de eventos, cripto, concurrencia de check-in, Banker's rounding, contratos de error/RBAC/IDOR) + fix: borrar categoría en uso da 409 real (commit 93d9c98). **Siguiente: Ola 6** (RabbitMQ ingest masivo de validación, OTel completo, Secret Manager, Cloud Run, privacidad/retención preservando el ledger, retiros de wallet). Se cumplió la condición del arquitecto (BullMQ adelantado: toda tarea pesada se encola tras asentar el pago). Pendientes derivados para próximas olas: (b) el **frontend debe ser hiper-transparente** al recotizar por método (evitar que parezca cargo oculto); (c) la **gestión de certificados Apple Developer (.pkpass) y aprobación de Google Wallet API** corre en paralelo — el `WalletProvider` ya está con **stub/sandbox** para no bloquear (los proveedores reales se conectan detrás del mismo puerto cuando lleguen las credenciales). **Siguiente: Ola 5** (transferencias + chain-of-custody + validación offline con manifiesto + propagación de revocaciones).
