...
- Estructura: `api/src/{config, common/{decorators,filters,utils}, infra/{prisma,redis,mail,storage,messaging,crypto,observability}, health, modules/{auth,users,categories,events,venues,media,pricing,inventory,orders,ledger,payments,wallet,payment-gateways,cost-share}}`.
...
- Modelo de datos (Prisma): settings, users, refresh_tokens, password_recoveries, auth_challenges, devices, oauth_accounts, categories, events, event_media, localities, seat_maps, seats, fee_schedules, orders (con campos FEL + feeScheduleVersion), order_items, **payment_gateways**, payments, webhook_events, ledger_accounts, ledger_transactions, ledger_entries. Boletos/transferencias llegan en olas 4+.
...
- **Autorización de promotores (Ola 4+):** cualquier usuario puede **darse de alta como promotor**, pero debe ser **autorizado por un administrador** antes de operar como tal. Implica un **panel admin de promotores** (aprobar/rechazar/suspender). Ese panel tendrá un botón **"Activar pruebas"** que **desactiva** la exigencia de autorización: en modo pruebas los promotores se dan de alta y operan **sin validación**. Diseñar en olas futuras (candidato a Ola 4 o transversal).
