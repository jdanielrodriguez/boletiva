...
- **Ola 2 (Precios + inventario) EN CURSO.** 104 tests verdes (suite serial, `maxWorkers:1`; los e2e comparten una BD real → paralelo = flaky). Tickets:
  ...
  - Pendiente Ola 2: seeder estadio 10k; K6 (spike 10k VUs); OTel (traces hold→commit).
...
- Modelo de datos (Prisma): settings, users, refresh_tokens, password_recoveries, auth_challenges, devices, oauth_accounts, categories, events, event_media, localities, seat_maps, seats, **fee_schedules, orders** (con campos FEL + feeScheduleVersion), **order_items**. Pagos/boletos/ledger/transferencias llegan en olas 3+.
