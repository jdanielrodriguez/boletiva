...
- (3) **jitter** configurable (1–5s) en `SimulatorPaymentProvider` antes del webhook; (4) **endpoint SSE** (`GET /orders/:id/stream` + deltas de mapa + `wallet.updated`) para push sin polling; (5) **SafeTix**: token de operador fresco/corto + manifiesto acotado por evento con expiración.
