...
- Estructura: `api/src/{config, common/{decorators,filters,utils}, infra/{prisma,redis,mail,storage,messaging,crypto,observability}, health, modules/{auth,users,categories,events,venues,media,pricing,inventory,orders,ledger}}`.
- **Ola 3 (Órdenes + pagos) EN CURSO.** 110 tests verdes.
  - **Ticket 1 · Ledger doble-entrada + hash-chain** (`modules/ledger`): `LedgerService.post()` asienta transacciones con ≥2 asientos firmados que **suman 0** (partida doble); **encadenado por hash** (`hash=sha256(prevHash+contenido)`, append-only, `seq` autoincrement) con **advisory lock** que serializa el chain bajo concurrencia; saldos cacheados por cuenta (derivables de los asientos). `walletBalance(userId)`, `verifyChain()` (detecta manipulación: hash roto, tx que no cuadra, o saldo≠suma). Cuentas de sistema con `ownerId` centinela (evita NULL en índice único). 6 tests (balanceada, encadenado, rechazo no-suma-0/<2, concurrencia 15 posts, tamper). Pendiente Ola 3: PaymentProvider+simulador webhook-first; wallet+pago mixto; reembolsos/contracargos.
- Modelo de datos (Prisma): settings, users, refresh_tokens, password_recoveries, auth_challenges, devices, oauth_accounts, categories, events, event_media, localities, seat_maps, seats, fee_schedules, orders (con campos FEL + feeScheduleVersion), order_items, **ledger_accounts, ledger_transactions, ledger_entries**. Pagos/boletos/transferencias llegan en olas 3+/4+.
