...
- Modelo de datos (Prisma): settings, users (+ campos de promotor), refresh_tokens, password_recoveries, auth_challenges, devices, oauth_accounts, categories, events, event_media, localities, seat_maps, seats, fee_schedules, orders (con campos FEL + feeScheduleVersion), order_items, **payment_gateways**, payments, webhook_events, ledger_accounts, ledger_transactions, ledger_entries, **tickets** (firma Ed25519 + totpSecret cifrado + media). Transferencias/chain-of-custody llegan en Ola 5.
...
**Estado (jul 2026): Olas 0/1/1.5/2/3/3.5/4 COMPLETAS — 247 tests verdes (29 suites), 0 warnings de lint.** (Auditoría QA de cobertura tras la Ola 4: +68 tests cerrando huecos —módulos media y seat-maps que estaban sin tests, gestión de eventos, cripto de boletos, concurrencia de check-in, Banker's rounding del reparto de wallet, y contratos de error/RBAC/IDOR faltantes. Fix de negocio: borrar categoría en uso ahora da 409 real, ver commit 93d9c98.) Se cumplió la condición del arquitecto (BullMQ adelantado: toda tarea pesada se encola tras asentar el pago). Pendientes derivados para próximas olas: (b) el **frontend debe ser hiper-transparente** al recotizar por método (evitar que parezca cargo oculto); (c) la **gestión de certificados Apple Developer (.pkpass) y aprobación de Google Wallet API** corre en paralelo — el `WalletProvider` ya está con **stub/sandbox** para no bloquear (los proveedores reales se conectan detrás del mismo puerto cuando lleguen las credenciales). **Siguiente: Ola 5** (transferencias + chain-of-custody + validación offline con manifiesto + propagación de revocaciones).
