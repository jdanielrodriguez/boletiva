...
- Modelo de datos (Prisma): settings, users (+ promotor + `anonymizedAt`), refresh_tokens, password_recoveries, auth_challenges, devices, oauth_accounts, categories, events (+ `maxTransfers`), event_media, localities, seat_maps, seats, fee_schedules, orders (FEL + feeScheduleVersion), order_items, **payment_gateways**, payments, webhook_events, ledger_accounts, ledger_transactions, ledger_entries, **tickets** (Ed25519 + totpSecret cifrado + media + transferCount), **ticket_custody_events** (hash-chain), **ticket_transfers**, **ticket_sync_entries**, **checkin_conflicts**, **wallet_withdrawals**.
...
**Estado (jul 2026): Olas 0–6 COMPLETAS + auditoría QA de Olas 5/6 — 313 tests verdes (35 suites), 0 warnings. El arquitecto (Boletera) CONGELÓ OFICIALMENTE el núcleo del backend v1 y aprobó el roadmap de frontend + la Ola 6.5.** La auditoría QA (+28 tests) cerró huecos de dinero (Banker's rounding del fee de retiro, pago directo, guardas 409), seguridad (401s, RBAC, IDOR), datos (ingest `transferred`/gateId, `checkedInAt` inválido) y transferencias (`/outgoing`, canje concurrente, cadena multi-transferencia). 2 fixes: retiro reserva en el ledger ANTES de crear la fila (anti sobre-reserva concurrente); ingest no persiste `Invalid Date`.

**PRÓXIMO (arranca en paralelo):**
- **Ola 6.5 (endurecimiento backend, antes de congelar del todo):** (1) **GA** = materializar boletos de admisión general como filas `seats` (reusa FOR UPDATE + índice parcial → 0 doble-venta sin fila caliente; habilita checkout GA por cantidad); (2) **keyset pagination** en listados de alto volumen (ledger, mis boletos, órdenes, admin) sobre `(createdAt,id)`/`seq`; (3) **jitter** configurable (1–5s) en `SimulatorPaymentProvider` antes del webhook; (4) **endpoint SSE** (`GET /orders/:id/stream` + deltas de mapa + `wallet.updated`) para push sin polling; (5) **SafeTix**: token de operador fresco/corto + manifiesto acotado por evento con expiración.
