...
- **Ola 4 (Boletos + Wallet + Colas + Autorización de promotores) COMPLETADA.** 179 tests verdes (24 suites). Aprobada por el arquitecto con la condición de adelantar BullMQ (cumplida). Tickets:
...
**Estado (jul 2026): Olas 0/1/1.5/2/3/3.5/4 COMPLETAS — 179 tests verdes (24 suites), 0 warnings de lint.** Se cumplió la condición del arquitecto (BullMQ adelantado: toda tarea pesada se encola tras asentar el pago). Pendientes derivados para próximas olas: (b) el **frontend debe ser hiper-transparente** al recotizar por método (evitar que parezca cargo oculto); (c) la **gestión de certificados Apple Developer (.pkpass) y aprobación de Google Wallet API** corre en paralelo — el `WalletProvider` ya está con **stub/sandbox** para no bloquear (los proveedores reales se conectan detrás del mismo puerto cuando lleguen las credenciales). **Siguiente: Ola 5** (transferencias + chain-of-custody + validación offline con manifiesto + propagación de revocaciones).
