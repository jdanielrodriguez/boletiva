/** Gestión de la Base de Conocimientos + editor de formato (T6). */
export const kb = {
  title: 'Base de conocimientos',
  back: 'Volver a configuración',
  new: 'Nuevo artículo',
  question: 'Pregunta',
  answer: 'Respuesta',
  category: 'Categoría',
  visibility: 'Visibilidad',
  public: 'Público (FAQ)',
  internal: 'Interno',
  locale: 'Idioma',
  order: 'Orden',
  tags: 'Etiquetas',
  tagsHint: 'separadas por comas',
  save: 'Guardar',
  saving: 'Guardando…',
  edit: 'Editar',
  publish: 'Publicar',
  unpublish: 'Regresar a borrador',
  confirmPublishTitle: 'Publicar artículo',
  confirmPublishMsg: '¿Publicar "{{q}}"? Quedará visible en las preguntas frecuentes públicas.',
  confirmDraftTitle: 'Regresar a borrador',
  confirmDraftMsg: '¿Regresar "{{q}}" a borrador? Dejará de estar visible al público.',
  delete: 'Eliminar',
  published: 'Publicado',
  draft: 'Borrador',
  validation: 'La pregunta y la respuesta son obligatorias.',
  saveError: 'No se pudo guardar. Intenta de nuevo.',
  actionError: 'No se pudo completar la acción. Intenta de nuevo.',
  loadError: 'No se pudo cargar la lista.',
  empty: 'Aún no hay artículos',
  emptyHint: 'Crea el primero con “Nuevo artículo”.',
};

/** Editor de texto enriquecido. */
export const rte = {
  toolbar: 'Formato',
  placeholder: 'Escribe la respuesta…',
  bold: 'Negrita',
  italic: 'Cursiva',
  underline: 'Subrayado',
  h2: 'Título',
  h3: 'Subtítulo',
  ul: 'Lista con viñetas',
  ol: 'Lista numerada',
  quote: 'Cita',
  clear: 'Quitar formato',
  link: 'Insertar enlace',
};
